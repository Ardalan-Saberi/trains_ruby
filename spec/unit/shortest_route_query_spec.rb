...
describe TrainsRuby::ShortestRouteQuery do
  ...
  before(:each) do
    @empty_rs, @single_road_rs, @small_city_rs, @disjoint_rs = double_railroads(TrainsRuby::ShortestRouteQuery)
  end

  describe "#get_shortest_route" do

    it "raise NoSuchRoute when given empty railroad system" do
      expect {@empty_rs.get_shortest_route("no_station1", "no_station2")}.to raise_error(TrainsRuby::NoSuchStationError)
    end

    it "raise NoSuchRouteError when either of stations doesn't exist in railroad system" do
      expect {@single_road_rs.get_shortest_route("no_station", "A")}.to raise_error(TrainsRuby::NoSuchStationError)
      expect {@single_road_rs.get_shortest_route("A", "no_station")}.to raise_error(TrainsRuby::NoSuchStationError)
    end
    ...
    it "should raise NoSuchRouteError when no routes between origin and destination" do
